from app.config import HUGGINGFACE_API_KEY, HUGGINGFACE_LLM_MODEL, GEMINI_API_KEY, GEMINI_MODEL
...
def generate_theory_answer(
    ...
    context = rerank_results(expanded_query, candidates, top_k=top_k)
    ...
    if not context_text:
        return "❌ No relevant content found. Please upload GATE study materials or textbooks first."

    # Build targeted prompt based on query classification
    if classification["is_summary"]:
        prompt = f"""You are an expert GATE exam tutor. Create a comprehensive yet concise summary covering the key concepts.

Structure your answer with:
• Main concepts and definitions
• Important formulas or algorithms
• Key points for GATE exam
...
Provide a well-organized summary focusing on GATE exam relevance:"""

    elif classification["is_explanation"]:
        prompt = f"""You are an expert GATE exam tutor helping a student understand a concept.
...
5. Highlight common mistakes students make
    6. Include formulas/diagrams if relevant (use LaTeX for formulas, ASCII diagrams if helpful)
    7. End with "Key points to remember"
    8. Provide multiple perspectives/approaches if available
...
Provide a clear, detailed explanation:"""
        ...
        prompt = f"""You are an expert GATE exam tutor explaining differences between concepts.
...
Provide a detailed comparison:"""
        ...
        prompt = f"""You are an expert GATE exam tutor explaining formulas and theorems.
...
5. Mention GATE applications
    6. Include formula in LaTeX and add diagram if useful
    7. End with "Key points to remember"
    8. Provide multiple perspectives/approaches if available
...
Provide the formula with detailed explanation:"""
        ...
        prompt = f"""You are an expert GATE exam tutor solving numerical problems.
...
Solve step-by-step:"""
        ...
        prompt = f"""You are an expert GATE exam tutor defining concepts.

Instructions:
1. Provide concise definition
2. Explain importance in GATE
3. Give 2-3 key characteristics
...
Provide the definition:"""
        ...
        prompt = f"""You are an expert GATE exam tutor. Answer the student's question clearly and concisely.
...
Provide a clear, focused answer:"""
    ...
    try:
        ...
        if classification["is_summary"]:
            return _extractive_summary(context_text, max_sentences=20)
        return _fallback_answer(context_text, question)
...
def generate_mcqs(
    ...
    def _generate_gate_questions_deterministic(num_questions: int) -> list:
        """Generate GATE-style questions using deterministic extraction"""
    ...
    try:
        ...
        while remaining > 0 and len(all_mcqs_ai) < count:
            ...
            prompt = f"""You are an expert GATE exam question paper setter with 15+ years of experience. Generate EXACTLY {current_chunk} high-quality GATE-style multiple choice questions from the provided study material.

🎯 GATE MCQ REQUIREMENTS:

QUESTION TYPES TO USE (vary across questions):
1. **Conceptual Understanding**: Test deep understanding of concepts
2. **Formula Application**: Apply formulas with numerical values
3. **Algorithm Analysis**: Analyze time/space complexity, correctness
4. **Statement Verification**: True/False combinations (I, II, III)
5. **Assertion-Reasoning**: Test logical relationships
6. **Scenario-Based**: Multi-step problem solving
7. **Comparison**: Compare techniques/algorithms/methods
8. **Output Prediction**: What will be the result?
9. **Error Detection**: Find the incorrect statement
10. **Optimization**: Best approach for given constraints
...
✅ Difficulty: 30% Easy, 40% Medium, 30% Hard (GATE level)
...
**Type 3 - Scenario:**
"A hash table of size 10 uses linear probing. Keys 23, 43, 13, 27 are inserted in order. If 37 is inserted next, collisions occur at positions:"
A) 3 only  B) 3, 4  C) 3, 4, 5  D) 7, 8

Now generate {current_chunk} GATE-standard questions from this context:
...
[
  {{
    "question": "Clear GATE-style question with specific details",
    "options": {{
...
Generate {current_chunk} diverse, GATE-level MCQs now (JSON only):"""
                                ...
                                for item in data:
                                    try:
                                        ...
                                        all_mcqs_ai.append(MCQQuestion(
                                            ...
                                            options=[
                                                MCQOption(label="A", text=str(options.get("A", "Option A"))),
                                                MCQOption(label="B", text=str(options.get("B", "Option B"))),
                                                MCQOption(label="C", text=str(options.get("C", "Option C"))),
                                                MCQOption(label="D", text=str(options.get("D", "Option D"))),
                                            ],
    ...
    try:
        fallback_mcqs = _generate_gate_questions_deterministic(count)
    except Exception as e:
        fallback_mcqs = []
